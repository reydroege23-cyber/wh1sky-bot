...
def _should_alert_owner(error_type: str) -> bool:
    """Check if enough time has passed to alert owner for this error type."""
    current_time = time.time()
    last_alert = _owner_alert_timestamps.get(error_type, 0)
    
    if current_time - last_alert >= OWNER_ALERT_COOLDOWN:
        _owner_alert_timestamps[error_type] = current_time
        return True
    return False
...
async def ask_ai(message: str, chat_context: str = "group", is_owner: bool = False) -> str:
    ...
    try:
        ...
        # Safe list indexing: check if choices is empty
        if not response.choices:
            logger.warning("AI choices list is empty")
            raise ValueError("AI returned empty choices list")
        
        text = response.choices[0].message.content.strip()
        
        # Check if text is empty
        if not text:
            logger.warning("AI returned empty text content")
...
async def ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_ping:
            await _notify_owner_of_error(context, f"Ping error: {str(e)[:100]}", chat_id=chat_id_ping, error_type="ping")
...
🎉 Enjoy using Marine!
        ...
        if is_group_upd:
            await _notify_owner_of_error(context, f"Update command error: {str(e)[:100]}", chat_id=chat_id_upd, error_type="update")
...
async def test_ai(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_test:
            await _notify_owner_of_error(context, f"AI test failed: {str(e)[:100]}", chat_id=chat_id_test, error_type="ai_test")
...
async def _send_marine_ai_response(
    ...
    try:
        ...
        if is_group:
            await _notify_owner_of_error(context, f"Marine AI failure: {str(e)[:100]}", chat_id=chat_id, user_id=user_id_int, error_type="marine_ai")
...
# =========================
# SAFE STICKER & MEDIA HANDLING
# =========================

async def handle_sticker_safely(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    """Safely handle stickers and animated/video stickers without crashes.
    
    Returns:
        True if sticker was processed, False otherwise.
    """
    ...
    try:
        ...
        # Skip animated and video stickers (can't analyze them safely)
        if sticker.is_animated or sticker.is_video:
            logger.debug(f"Skipping animated/video sticker from user {user_id} safely")
            return True
        
        # Static stickers pass through silently
        logger.debug(f"Static sticker from user {user_id}")
        return True
        
    except AttributeError as e:
        logger.debug(f"Sticker attribute error (expected for some sticker types): {e}")
        return True
    except Exception as e:
        logger.debug(f"Sticker handling error: {e}")
        # Don't alert owner for sticker errors - they're normal
        return True
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not update.message or not update.message.text or not update.effective_chat or not update.effective_user:
        # Try to handle stickers safely
        if update.message and update.message.sticker:
            await handle_sticker_safely(update, context)
        return
    if getattr(update.effective_user, "is_bot", False):
    ...
    try:
        ...
        if text.startswith("/ai"):
            ...
            try:
                ...
                if is_group_inner:
                    await _notify_owner_of_error(context, f"AI handler error: {str(e)[:100]}", chat_id=chat_id_inner, error_type="ai_handler")
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"Message handler error: {str(e)[:100]}", chat_id=chat_id, user_id=user_id, error_type="message_handler")
    ...
    try:
        ...
        if new_count >= MAX_WARNINGS:
            ...
            try:
                ...
                if is_group_ban:
                    await _notify_owner_of_error(context, f"Ban failed for {username}: {str(ban_error)[:100]}", chat_id=chat_id_ban, user_id=user_id, error_type="ban")
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"Warn command error: {str(e)[:100]}", chat_id=chat_id, user_id=user_id, error_type="warn")
@admin_only
...
async def mute(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_mute:
            await _notify_owner_of_error(context, f"Mute error: {str(e)[:100]}", chat_id=chat_id_mute, error_type="mute")
...
async def kick(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_kick:
            await _notify_owner_of_error(context, f"Kick error: {str(e)[:100]}", chat_id=chat_id_kick, error_type="kick")
...
async def ban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        is_group_ban = chat_id_ban is not None and int(chat_id_ban) < 0
        if is_group_ban:
            await _notify_owner_of_error(context, f"Ban error: {str(e)[:100]}", chat_id=chat_id_ban, error_type="ban")
...
async def unban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_unban:
            await _notify_owner_of_error(context, f"Unban error: {str(e)[:100]}", chat_id=chat_id_unban, error_type="unban")
...
async def foreverban(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if is_group_fb:
            await _notify_owner_of_error(context, f"Foreverban error: {str(e)[:100]}", chat_id=chat_id_fb, error_type="foreverban")
...
Warnings: {warns}/{MAX_WARNINGS}
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"User info error: {str(e)[:100]}", chat_id=chat_id, error_type="user_info")
...
async def debug_warns(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"Debug warns error: {str(e)[:100]}", chat_id=chat_id, error_type="debug_warns")
...
async def guess_game(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"Guess game error: {str(e)[:100]}", chat_id=chat_id, error_type="guess_game")
...
async def truth(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    truths = [
        ...
        "What's the most embarrassing thing you've done with your family?",
        ...
        "Have you ever been caught making a weird face in the mirror?",
        ...
        "Have you ever been caught doing something weird in your sleep?",
        "What's the most embarrassing thing you've done while trying to take a selfie?",
        "What's the most embarrassing thing you've done while trying to dance?",
        "Have you ever been caught doing something weird in public?",
        "What's the most embarrassing thing you've done while trying to flirt?",
        "What's the most embarrassing thing you've done while trying to be funny?",
        "Have you ever been caught doing something weird on a video call?",
        ...
        "Have you ever been caught doing something weird while alone?",
        "What's the most embarrassing thing you've done while trying to be mysterious?",
        "What's the most embarrassing thing you've done while trying to be confident?",
        "Have you ever been caught doing something weird while with friends?",
        "What's the most embarrassing thing you've done while trying to be popular?",
        "What's the most embarrassing thing you've done while trying to be rebellious?",
        "Have you ever been caught doing something weird while with family?",
        "What's the most embarrassing thing you've done while trying to be unique?",
        "What's the most embarrassing thing you've done while trying to be yourself?"
    ]
...
async def dare(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    dares = [
        ...
        "Pretend to be a chef and give a recipe for a delicious meal, then send a voice message of it.",
        "Try to do a backflip and send a video of it.",
        "Send a voice message of you doing your best impression of a famous musician.",
        "Pretend to be a motivational speaker and give a speech about overcoming challenges, then send a voice message of it.",
        "Try to do a cartwheel and send a video of it.",
        "Send a voice message of you doing your best impression of a famous comedian.",
        "Pretend to be a fashion model and give a runway walk, then send a video of it.",
        "Try to do a split and send a video of it.",
        "Send a voice message of you doing your best impression of a famous athlete.",
        "Pretend to be a news reporter and give a report on a recent event, then send a voice message of it.",
        "Try to do a handstand on a wall and send a video of it.",
        "Send a voice message of you doing your best impression of a famous TV character.",
        "Pretend to be a stand-up comedian and tell a joke, then send a voice message of it.",
        "Try to do a headstand and send a video of it.",
        "Send a voice message of you doing your best impression of a famous cartoon character.",
        "Pretend to be a DJ and give a mix of your favorite songs, then send a voice message of it.",
        "Try to do a moonwalk and send a video of it.",
        "Send a voice message of you doing your best impression of a famous video game character.",
        "Pretend to be a sports commentator and give commentary on a recent game, then send a voice message of it.",
        "Try to do a handstand on a table and send a video of it.",
        "Send a voice message of you doing your best impression of a famous movie character.",
        "Pretend to be a magician and perform a magic trick, then send a video of it.",
        "Try to do a backflip off a couch and send a video of it.",
        "Send a voice message of you doing your best impression of a famous internet personality.",
        "Pretend to be a motivational speaker and give a speech about achieving your dreams, then send a voice message of it.",
        "Try to do a handstand on a skateboard and send a video of it.",
        "Send a voice message of you doing your best impression of a famous cartoon character.",
        "Pretend to be a chef and give a cooking demonstration, then send a video of it.",
        "Try to do a front flip and send a video of it.",
        "Send a voice message of you doing your best impression of a famous historical figure.",
        "Pretend to be a news anchor and give a report on a recent event, then send a voice message of it.",
        "Try to do a handstand on a chair and send a video of it.",
        "Send a voice message of you doing your best impression of a famous musician.",
        "Pretend to be a life coach and give advice on how to be happy, then send a voice message of it.",
        "Try to do a handstand on a wall and send a video of it.",
        "Send a voice message of you doing your best impression of a famous actor.",
        "Pretend to be a fashion model and give a runway walk, then send a video of it.",
...
async def iq_rate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"IQ check error: {str(e)[:100]}", chat_id=chat_id, error_type="iq_check")
...
{drip_comments[drip_rating - 1] if drip_rating <= len(drip_comments) else random.choice(drip_comments)}
        ...
        is_group = chat_id is not None and int(chat_id) < 0
        if is_group:
            await _notify_owner_of_error(context, f"Drip check error: {str(e)[:100]}", chat_id=chat_id, error_type="drip_check")
